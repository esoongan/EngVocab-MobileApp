...
import kotlinx.android.synthetic.main.fragment_to_kor.*
import java.util.*
...
class ToKorFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_to_kor, container, false)
    }
    ...
    private fun showQuestion() {     // 문제 보여주는 함수
        quiz_words.text = array2[currentNumber]    // 한글뜻 들어가있는 배열 array2을 순차로 보여줌
    }
    ...
    private fun getRandom() {    // 보기 만드는 함수
        ...
        for (count in 0..2) {
            val random = Random()
            val randomIndex = random.nextInt(array1.size - 1)  // 한글의 개수중에서 랜덤하게 뽑아서
            val wrongAnswer = array1[randomIndex]      // 그 랜덤한 인덱스값의 문자를 줘
            array1.removeAt(randomIndex) // 중복되면 안되니까 뽑은건 지우고
            arrayEx.add(wrongAnswer)     // 어레이 배열에 에 롱앤설을 넣어
        }
        var answer = array1[currentNumber] // 정답을 넣어줄 변수생성
        arrayEx.add(answer)  // 영인 array1에서 현재한글의 해당 영어뜻인 정답도 넣어줘야함
    ...
    private fun init() {
        ...
        adapter.itemClickListener = object : MyAdapter.OnItemClickListener {
            ...
            override fun OnItemCLick(
                ...
                if (wordsEtoK[data] == quiz_words.text) {    // 클릭한 값의 한글뜻과
